...
``offset (B,)`` (PTv3's native layout), to the fixed-length tokenized latent
distribution ``Z`` consumed by the RF velocity network as conditioning.
...
class PCEncoder(nn.Module):
    ...
    def _to_data_dict(
        ...
        """Build the PTv3 input dict from a packed point cloud.

        Args:
            coord: ``(P_sum, 3)`` concatenated points of the whole batch.
            offset: ``(B,)`` cumulative per-sample point counts (PTv3 convention;
                ``offset2batch`` recovers each point's sample id).
        """
